...
| **每日推荐推送** | 晨推（09:00 推荐股票）+ 午推（15:30 大盘分析），时间可配置 |
...
- **09:00**（晨推）：为每个用户推荐当日值得关注的股票 + 操作建议
- **15:30**（午推）：推送大盘收盘行情分析 + 操作建议

> 晨推/午推时间均可在管理后台「推送设置」中独立配置，各自有独立开关。
...
- **仪表盘** — 用户数、对话量、系统状态
- **用户管理** — 用户列表、编辑画像、对话记录、Bot 绑定
- **策略配置** — 养家/中长线选股参数
- **推送设置** ★新 — 推送开关、推送时间配置、手动测试推送
...
# ═══════ 2. 停旧服务 ═══════
fuser -k 8900/tcp     # 杀 admin
pkill cc-connect       # 杀微信桥接

# ═══════ 3. 启 admin（后台运行）═══════
cd /opt/stock-advisor
nohup .venv/bin/python -m cli admin >> data/logs/admin.log 2>&1 &

# ═══════ 4. 等 5 秒 确认起来了 ═══════
sleep 5
tail -20 data/logs/admin.log
fuser 8900/tcp    # 应打印 pid

# ═══════ 5. 启 cc-connect（后台运行）═══════
nohup cc-connect >> data/logs/cc-connect.log 2>&1 &
sleep 3
pgrep -a cc-connect   # 应看到 cc-connect 进程

# ═══════ 6. 浏览器配置 Tushare（可选）═══════
# 打开 http://服务器IP:8900 → 配置管理
#   Tushare Token       填你的 token
#   Tushare API 地址    填 API 地址
# 点保存 → 立即生效，不用再重启
```

> **注意**：定时推送（晨推 + 午推）运行在 admin 进程中。如果 admin 未启动，推送不会执行。
